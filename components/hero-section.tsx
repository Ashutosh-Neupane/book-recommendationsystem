...
export default function HeroSection({ userName }: HeroSectionProps) {
    ...
    const fetchTopRatedBooks = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // ✅ FIXED: Ensure each book has an id field
          const booksWithId = books.map((book: any) => ({
            ...book,
            id: book.id || book._id, // Use id if available, fallback to _id
          }));

          setTopRatedBooks(booksWithId);
          console.log("Fetched books:", booksWithId); // Debug log
        } else {
  ...
  useEffect(() => {
    if (topRatedBooks.length > 0) {
      const totalSlidesCalculated = Math.ceil(topRatedBooks.length / 4);
      const interval = setInterval(() => {
        setCurrentSlide((prev) => (prev + 1) % totalSlidesCalculated);
      }, 5000);
          ...
          <div className="text-center space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-12">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="h-80 bg-white/20 rounded-xl animate-pulse"
                />
              ))}
            </div>
          </div>
  ...
  // ✅ FIXED: Handle empty books array
  if (topRatedBooks.length === 0) {
    return (
      <div className="relative overflow-hidden bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 text-white">
        <div className="absolute inset-0 bg-black/20" />
        <div className="relative container mx-auto px-4 py-20">
          <div className="text-center space-y-6">
            ...
            <p className="text-lg md:text-xl text-blue-100 max-w-3xl mx-auto leading-relaxed">
              No top-rated books available at the moment. Check back soon!
            </p>
            <Button
              onClick={() => router.push("/search")}
              className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white px-8 py-4 rounded-full font-semibold text-lg transition-all duration-300 hover:scale-105"
            >
              Browse All Books
            </Button>
          </div>
  ...
  const adjustedCurrentSlide = Math.min(currentSlide, totalSlides - 1);
  const currentBooks = topRatedBooks.slice(
    adjustedCurrentSlide * booksPerSlide,
    (adjustedCurrentSlide + 1) * booksPerSlide
  );
    ...
    <div className="relative overflow-hidden bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 text-white">
      ...
      <div className="relative container mx-auto px-4 py-20">
        <div className="text-center space-y-6 mb-16">
          ...
          <p className="text-lg md:text-xl text-blue-100 max-w-3xl mx-auto leading-relaxed">
            Discover our highest-rated literary treasures, curated just for
            passionate readers like you
          </p>
          ...
          <div className="overflow-hidden">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {currentBooks.map((book, index) => (
                <Card
                  key={book.id} // ✅ FIXED: Now uses the proper id field
                  ...
                  onClick={() => {
                    console.log("Clicking book with id:", book.id); // Debug log
                  ...
                  <CardContent className="p-0">
                    <div className="relative">
                      ...
                      <div className="absolute top-4 left-4 z-20 bg-gradient-to-r from-yellow-400 to-orange-500 text-black px-3 py-1 rounded-full text-xs font-bold shadow-lg">
                        #{adjustedCurrentSlide * booksPerSlide + index + 1}
                      </div>
                      ...
                      {/* Book Cover with improved FallbackImage */}
                      <div className="relative h-80 overflow-hidden">
                        <FallbackImage
                          ...
                          priority={index < 4} // ✅ FIXED: Only prioritize first 4 images
                        />
              ...
              {Array.from({ length: totalSlides }).map((_, index) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-all duration-300 ${
                    index === adjustedCurrentSlide ? "bg-white" : "bg-white/30"
                  }`}
